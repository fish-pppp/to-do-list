...
On a personal Vercel deploy, Settings → Sync & Backup can also keep one cloud backup in Vercel Blob (`/api/backup`). The same sync key must match the project's `SYNC_KEY`. When this browser is empty and a backup exists, the app restores it; after local edits it saves again. This does not replace opening the same production URL on every device.
